...
验证段的叙述者是提交者本人。历史改写场景中，改写者的取证过程按 §13 记入 Git 决策日志，不进入 message。真实正反例（反例取自未采用的重写草稿）：

```text
不通过：验证：未找到当期独立执行记录；仅核对了新增测试断言。测试代码存在不代表这些断言当期已经执行。
通过：  验证：go test ./internal/reliability/ 通过，覆盖 65536-token 批装满放行与后续 1000-token 偿还等待两项断言。
通过：  验证：（原提交所记）Tests: 18 包全量通过。
```

第一种把改写者的取证处境写进了提交者口吻的正文；后两种分别对应"有当期记录"与"仅有原提交记录"两个合法形态，没有记录时省略验证段并在决策日志说明。
...
| 版本 | 变更 |
|---|---|
| 2.2.2 | §9.3 增加验证段真实正反例：明确叙述者为提交者本人，历史改写的取证过程按 §13 记入决策日志；给出有当期记录与仅有原提交记录两个合法形态 |
